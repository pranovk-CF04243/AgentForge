...
import { AlertOctagon, CheckCircle, Cpu, Loader2 } from 'lucide-react';

export const IncidentWarRoom: React.FC = () => {
  const incidents = useStore((state) => state.incidents);
  const resolveIncident = useStore((state) => state.resolveIncident);
  const resolveAllIncidents = useStore((state) => state.resolveAllIncidents);
  ...
  const handleResolve = async () => {
    ...
    try {
      if (activeIncidents.length > 1) {
        ...
        await resolveIncident(currentInc.id);
      }
    } finally {
    ...
    <div className="bg-rose-700 dark:bg-rose-950/90 border-b border-rose-600 dark:border-rose-600/80 px-4 py-2.5 flex flex-wrap items-center justify-between gap-3 text-white backdrop-blur-md z-20 animate-in slide-in-from-top duration-300 shadow-md">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {activeIncidents.length > 1 && (
              <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-200 border border-amber-400/40 text-[10px] font-mono font-bold">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 dark:bg-emerald-600 dark:hover:bg-emerald-500 text-slate-950 font-bold text-xs font-mono transition-colors shadow-lg disabled:opacity-50"
        >
          {isResolving ? (
          ...
          <span>{isResolving ? 'Resolving...' : activeIncidents.length > 1 ? 'Execute Automated Remediation (All)' : 'Execute Automated Remediation'}</span>
        </button>
      </div>
    </div>
  );
